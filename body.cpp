...
GravitationalBody:: GravitationalBody(int ID, valtype mass, vector<valtype> position, vector<valtype> momentum, vector<valtype> velocity): ID(ID), mass(mass), position(position), momentum(momentum), velocity(velocity){}
...
GravitationalSystem::  GravitationalSystem(string&& infile, valtype progTime): progTime(progTime){
    ...
    while(!instream.eof()){
        ...
        bodies.push_back(GravitationalBody(ID,m,{x,y,z},{vx*m,vy*m,vz*m}, {vx, vy, vz}));
    }
}
